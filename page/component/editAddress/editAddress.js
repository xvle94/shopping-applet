...
Page({
  data: {
    ...
    areaList:[],
    multiArray: [],  // 三维数组数据
    multiIndex: [0, 3, 0], // 默认的下标
    step: 0, // 默认显示请选择
    currnetCityQueryId:'',//市级id
    pCode:'',
    address:{}
  },
  ...
  formSubmit(e){
    ...
    //判断电话号格式是否正确
      ...
      app.postReq('/api/user/address/edit',address,(res)=>{
        console.log(res);
        if(res.code == 200){
          wx.showToast({
            title: '保存成功！',
    ...
    app.getReq('api/user/queryByPid',param,(res)=>{
      ...
      that.setData({
       ...
       // pCode: provinceList[0].Code // 用于查询收费站点的查询code，添加清空项后不需要该操作，都可
        ...
        let province = that.data.address.province;
        let index = 0;
        for(var i in that.data.multiArray){
          for(var k in that.data.multiArray[i]){
            if(that.data.multiArray[i][k] == province){
              console.log(k);
              index = k;
              break;
            }
        ...
        let currnetCityQueryId = that.data.provinceList[index].id
        var multiIndex = that.data.multiIndex;
        that.setData({
          'multiIndex[0]':index,
          pid:that.data.provinceList[index].id
        })
        that.getCity(currnetCityQueryId);
      })
  ...
  getCity(id) { // 获取市级数据
    ...
    app.getReq('api/user/queryByPid',param,(res)=>{
      ...
      that.setData({
        ...
        let city = that.data.address.city;
        let index = 0;
        for(var i in that.data.multiArray){
          for(var k in that.data.multiArray[i]){
            if(that.data.multiArray[i][k] == city){
              index = k;
              break;
            }
          }
        }
        let currnetCityQueryId = that.data.cityList[index].id
        var multiIndex = that.data.multiIndex;
        that.setData({
          'multiIndex[1]':index,
          cid:that.data.cityList[index].id
        });
        that.getArea(currnetCityQueryId);
      })
  ...
  getArea(id) {
    ...
    app.getReq('api/user/queryByPid',param,(res)=>{
      ...
      var areaArr = data.map((item) => { return item.cityName })
        ...
        let county = that.data.address.county;
        let index = 0;
        for(var i in that.data.multiArray){
          for(var k in that.data.multiArray[i]){
            if(that.data.multiArray[i][k] == county){
              index = k;
              break;
            }
          }
        ...
        that.setData({
          'multiIndex[2]':index,
          aid:that.data.areaList[index].id
        });
      })
    })
  },
  ...
  getDetail(){
    ...
    app.getReq('/api/user/address/info',query,(res) => {
      console.log(res);
      setTimeout(function () {
        wx.hideLoading()
      }, 1000)
      ...
      that.setData({
        address:res.result,
        items:items,
        switch1Checked:res.result.defaultStatus
      },()=>{
  ...
  delAddress(){
    ...
    app.getReq('/api/user/address/del',id,(res)=>{
      if(res.code == 200){
        wx.showToast({
          title: '删除成功！',
          icon: 'none',
